...
fn ripple_field(world: vec2<f32>) -> vec2<f32> {
    ...
    for (var i = 0u; i < RIPPLE_SLOTS; i = i + 1u) {
        ...
        if (strength > 0.001 && age >= 0.0 && age < 7.0) {
            let delta = world - r.xy;
            let dist = max(length(delta), 0.001);
            let dir = delta / dist;
            let wave = sin(dist * 0.022 - age * 3.5);
            let falloff = exp(-dist * 0.006) * exp(-age * 0.38);
            offset = offset + dir * wave * falloff * strength * 75.0;
        }
    }
    return offset;
}
...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let wake = saturate(length(displacement) / 8.0);

    let base = vec3<f32>(0.002, 0.010, 0.032);
    let grid_bright = 1.0 + material.shard_glow * 0.40;
    let grid = vec3<f32>(0.025, 0.18, 0.54) * (points + lines) * shimmer * grid_bright;
    let foam = vec3<f32>(0.14, 0.60, 1.0) * wake * (0.30 + points * 0.60);
    var color = vec4<f32>(base + grid + foam, board_mask * saturate(0.10 + points * 0.45 + lines + wake * 0.55));
