...
fn diagonal_fall_target_in(
    ...
    let below = (pos.x, pos.y - 1);
    let below_is_static_blocker = shadow_set.contains(&below);
    if !below_is_static_blocker && !occupied.contains(&below) {
        return None;
    }
    ...
    for dx in [-1, 1] {
        ...
        // A piece resting on a static blocker should be able to spill into an exposed
        // diagonal cell immediately. Waiting for that target column's eventual vertical
        // refill leaves a visually obvious hole beside the blocker (notably after opening
        // the central hard-shadow cells in level 3).
        //
        // When the source is resting on an ordinary movable piece, keep the vertical-feed
        // guard: otherwise normal stacks zigzag sideways during every refill.
        if !below_is_static_blocker && !vertical_feed_blocked_for_target(diag, occupied, shadow_set)
        {
            continue;
        }
        return Some(GridPos {
...
pub(crate) fn apply_gravity(
    ...
    for (e, pos, vis, is_spark) in &sorted {
        ...
        if let Some(target) =
            portal_target.or_else(|| straight_fall_target_in(*pos, &occupied, shadow_set, &layout))
        {
            ...
            if let Ok((_, mut p, mut visual, mut transform, _)) = entities.get_mut(*e) {
                ...
                if portal_target.is_some() {
                    ...
                    commands
                        .entity(*e)
                        .insert(FallSpeed((portal_entry - destination).length() / 0.18));
                }
            }
...
mod tests {
    ...
    fn validate_input_query_disjointness(
        mut lights: Query<&mut GridPos, (With<Light>, Without<Spark>, Without<BlocksInteraction>)>,
        mut sparks: Query<&mut GridPos, (With<Spark>, Without<Light>, Without<BlocksInteraction>)>,
    ...
    fn gravity_accepts_sparks_but_excludes_locked_lights() {
        ...
        app.world_mut()
            .spawn((Spark, FallPhysics, GridPos { x: 1, y: 4 }));
        app.world_mut()
            .spawn((Light, Stasis, BlocksGravity, GridPos { x: 1, y: 3 }));
    ...
    #[test]
    fn slides_around_static_blocker_even_when_target_has_vertical_feed() {
        let next = choose_fall_target(
            GridPos { x: 3, y: 4 },
            &occupied(&[(3, 4)]),
            &shadows(&[(3, 3)]),
