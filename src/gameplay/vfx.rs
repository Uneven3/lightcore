...
pub(crate) fn stage_power_impact_jelly(
    ...
    for (index, activation) in activations.iter().enumerate() {
        ...
        let base_delay =
            destroyed_edge_distance / settings.speed + settings.pop_duration + JELLY_AFTER_POP_SECS;
        for (&pos, &(entity, _, _)) in grid {
            ...
            let delay =
                base_delay + ((dx + dy - outer_radius).max(0) as f32) * SUPERNOVA_JELLY_STAGGER;
...
/// short held beat (`MatchTiming::combo_hold_secs`) — do they all detonate together in one
