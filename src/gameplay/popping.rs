...
/// so alpha can be faded independently per entity. Children are NOT despawned here: GlowPool halos
/// stay alive, and `visuals::core_motion::fade_cores_on_pop` fades the nucleus over this same timer,
/// so the light remains a coherent visual unit until its parent is finally despawned.
