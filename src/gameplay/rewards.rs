...
use super::{
    CaptureBatch, CapturedCore, CollectedCores, CoreReserve, MovesLeft, PowerActivation,
    PowerActivationQueue, PowerCreated, Score, ScoreDrained, StatsBook, SuperComboPending,
};
use crate::board::clear_shadow_at;
use crate::core::components::{AdjacentMatchDamage, HardShadow, Light, PopAnim, Spark};
use crate::core::grid::{GridPos, to_world};
...
fn capture_units(run: &RunState, kind: LightKind, cascade: u32, score_reset: bool) -> u32 {
    cascade
        + if score_reset {
            0
        } else {
            run.star_capture_bonus(kind, cascade)
        }
}

/// Number of visible reward echoes emitted for one captured core. This is a domain consequence of
/// active boons, published on `CaptureBatch` so presentation never has to inspect `RunState`.
fn feedback_copies(
    run: &RunState,
    color: LightColor,
    kind: LightKind,
    ...
    let captured = capture_units(run, kind, cascade, false);
    let star_copies = captured / cascade.max(1);
    run.reward_feedback_multiplier(color) * star_copies
}
...
pub(super) fn apply_removal_rewards(
    ...
    for e in to_remove {
        if let Some((_, color, kind)) = entity_info.get(e) {
            ...
            let captured = capture_units(economy.run, *kind, cascade, score_reset);
            economy.collected_cores.0[color.index()] += captured;
            if !score_reset {
                ...
                let star_bonus = captured - cascade;
            ...
            match color {
                LightColor::Red => economy.stats.reds += captured,
                LightColor::Green => economy.stats.greens += captured,
                LightColor::Blue => economy.stats.blues += captured,
                LightColor::Yellow => economy.stats.yellows += captured,
                LightColor::Purple => economy.stats.purples += captured,
            }
            if kind.is_power() {
...
/// captured-core records `CaptureBatch` needs to spawn score shards — shared by every
...
pub(super) fn spawn_pops(
    ...
    for e in to_remove {
            ...
            && !kind.is_hollow()
        {
            ...
            pops.push(CapturedCore {
                grid_position: *pos,
                color: *color,
                kind: *kind,
                available_after_secs: delay,
                capture_units: capture_units(run, *kind, cascade, score_reset),
                feedback_copies: feedback_copies(run, *color, *kind, cascade, score_reset),
            });
        }
    }
    pops
...
pub(crate) fn resolve_match_sequence(
    ...
    let removed_positions: HashSet<GridPos> = to_remove
        ...
        .collect();
    clear_shadow_at(&removed_positions, commands, shadow_q, shadow_count);

    let power_bonus = economy.run.power_bonus(upgrades.len() as u32);
    apply_removal_rewards(
        commands,
        &to_remove,
        entity_info,
        cascade_depth,
