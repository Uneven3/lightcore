...
use crate::presentation::GameLayout;
...
impl Plugin for MainMenuPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(Screen::MainMenu), spawn_main_menu)
            ...
            .add_systems(
                Update,
                (despawn_main_menu, ApplyDeferred, spawn_main_menu)
                    .chain()
                    .run_if(
                        resource_changed::<GameLayout>
                            .and_then(in_state(Screen::MainMenu))
...
fn spawn_main_menu(
    ...
    profile: Option<Res<PlatformProfile>>,
    ...
    let show_desktop_options = profile.as_ref().map_or(
        cfg!(not(any(target_os = "android", target_os = "ios"))),
        |profile| profile.show_desktop_options,
    );
    ...
    let play_icon = asset_server.load(crate::embedded::play_icon_path());
    let settings_icon = asset_server.load(crate::embedded::settings_icon_path());
    let power_icon = asset_server.load(crate::embedded::power_icon_path());
    commands
        .spawn((
            MainMenuRoot,
            Node {
