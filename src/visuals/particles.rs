...
use crate::gameplay::{LightTeleported, ManualLightEliminated};
...
pub(crate) fn on_manual_light_eliminated(
    trigger: On<ManualLightEliminated>,
    ...
    spawn_burst(
        ...
        to_world(trigger.pos),
        trigger.color.glow_color(),
        particles.pop_burst_count,
        particles.burst_radius,
    );
}

pub(crate) fn on_light_teleported(
    trigger: On<LightTeleported>,
    mut commands: Commands,
    cache: Res<VisualCache>,
    particles: Res<ParticleSettings>,
) {
    let image = cache.light_core_image(trigger.color);
    let color = trigger.color.glow_color();
    let count = particles.pop_burst_count.max(4);
    for position in [trigger.from, trigger.to] {
        spawn_burst(
            &mut commands,
            image.clone(),
            to_world(position),
...
pub(crate) fn tick_particles(
    ...
    for (e, mut t, mut sprite, mut velocity, mut lifetime, drag, force, origin) in &mut q {
        ...
        if frac >= lifetime.fade_start_frac {
            let fade =
                (frac - lifetime.fade_start_frac) / (1.0 - lifetime.fade_start_frac).max(0.0001);
