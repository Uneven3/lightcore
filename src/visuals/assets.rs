...
/// `breathe` / halo animation / particle-fade mutate every frame are still created at spawn time —
...
pub(crate) struct VisualCache {
    ...
    /// Shared textures for the per-instance-tinted sprites (cores + glow pools). `core_image` is a
    /// soft solid disc and `glow_image` is a radial halo falloff.
...
pub(crate) fn publish_board_visuals(mut commands: Commands, cache: Res<VisualCache>) {
    commands.insert_resource(BoardVisuals {
        shadow_mesh: cache.shadow_mesh.clone(),
        shadow_mat: cache.shadow_mat.clone(),
        hard_shadow_mat: cache.hard_shadow_mat.clone(),
        blocker_mesh: cache.blocker_mesh.clone(),
        blocker_mat: cache.blocker_mat.clone(),
        spark_mesh: cache.spark_mesh.clone(),
        spark_mat: cache.spark_mat.clone(),
        core_image: cache.core_image.clone(),
        glow_image: cache.glow_image.clone(),
        grid_cell_image: cache.grid_cell_image.clone(),
    });
}
...
/// driven entirely by the stable `Sprite::color`; baking a hue into the texture too would tint it
/// twice.
...
pub(crate) fn build_cache(
    ...
    let ring_mesh =
        std::array::from_fn(|i| meshes.add(build_light_color_mesh(LightColor::from_index(i))));
