...
///
/// `sync_halo_settings` reads this resource every frame, so dragging the Options sliders retunes
/// the live board.
...
/// is, so `sync_halo_settings` can recompute the tinted color/size live from `GlowSettings`.
#[derive(Component)]
pub(crate) struct GlowPool {
    layer: GlowLayer,
    /// The light's untinted color at brightness 1.0 — settings scale it without temporal modulation.
    raw: Srgba,
}
...
pub(crate) fn attach_glow_pools(
    ...
    new_lights: Query<(Entity, &LightColor, &LightKind), Added<Light>>,
) {
    for (e, color, kind) in &new_lights {
        let Srgba {
            red, green, blue, ..
...
/// Keeps every normal light halo stable. Re-reads `GlowSettings` each frame so Options can retune
/// the live board without introducing an animation that can be perceived as grid-wide flicker.
pub(crate) fn sync_halo_settings(
    settings: Res<GlowSettings>,
    ...
    for (mut sprite, pool, mut tf) in &mut q {
        ...
        sprite.color = tint(pool.raw, settings.brightness, alpha);
