...
use crate::gameplay::shop::MoveDragPreview;
...
/// Presentation-only drag preview for the paid Move special. The authoritative `GridPos` remains
/// untouched until drop; removing the marker restores the transform from `VisualPos`, so invalid
/// drops and cancellation cannot leave a light stranded under the pointer.
pub(crate) fn update_move_drag_preview(
    pointer: Res<PointerInput>,
    mut dragging: Query<&mut Transform, With<MoveDragPreview>>,
    mut removed: RemovedComponents<MoveDragPreview>,
    mut settled: Query<(&VisualPos, &mut Transform), Without<MoveDragPreview>>,
) {
    if let Some(world) = pointer.position_world {
        for mut transform in &mut dragging {
    ...
    for entity in removed.read() {
        if let Ok((visual, mut transform)) = settled.get_mut(entity) {
            transform.translation = visual.0;
        }
    }
}
