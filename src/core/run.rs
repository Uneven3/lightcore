...
impl RunState {
    ...
    /// Reward echoes make value boons readable before the projected counter catches up. The
    /// presentation adapter decides how an echo looks; the run domain only publishes its count.
    pub(crate) fn reward_feedback_multiplier(&self, color: LightColor) -> u32 {
        match color {
            LightColor::Red => 1 + self.level(BoonKind::RedValue) as u32,
            _ => 1,
        }
    }
...
fn load_run_progress(mut run: ResMut<RunState>, mut reserve: ResMut<CoreReserve>) {
    let Some(saved_run) = storage::load_save_file("run.txt").and_then(|raw| RunState::decode(&raw))
    else {
        return;
    };
...
fn sync_run_reserve(mode: Res<GameMode>, mut run: ResMut<RunState>, reserve: Res<CoreReserve>) {
...
mod tests {
    ...
    fn selling_refunds_the_exact_rank_price() {
        ...
        assert!(run.buy(BoonKind::RedValue));
        assert_eq!(
            run.sell(BoonKind::RedValue),
            Some(BoonKind::RedValue.cost(1))
        );
        assert_eq!(run.level(BoonKind::RedValue), 1);
        assert_eq!(
            run.sell(BoonKind::RedValue),
            Some(BoonKind::RedValue.cost(0))
        );
    ...
    #[test]
    fn red_value_increases_reward_feedback() {
        let mut run = RunState::default();
        run.buy(BoonKind::RedValue);
        run.buy(BoonKind::RedValue);

        assert_eq!(run.reward_feedback_multiplier(LightColor::Red), 3);
        assert_eq!(run.reward_feedback_multiplier(LightColor::Blue), 1);
    }
